...
video = {
    ...
    "chatgpt_template": """我将发给你多行文本,你将每行内容对应翻译为一行{lang},如果该行无法翻译,则将该行原内容作为翻译结果,如果是空行,则将空字符串作为结果,然后将翻译结果按照原顺序返回。请注意必须保持返回的行数同发给你的行数相同,比如发给你3行文本,就必须返回3行.不要忽略空行,不要确认,仅返回翻译结果,不要包含原文本内容,不要道歉,不要重复述说。请严格按照要求的格式返回,这对我的工作非常重要""",
}
# 存放 edget-tts 角色列表
