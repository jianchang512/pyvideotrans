...
class WorkerTTS(QThread):
    ...
    def __init__(self, *,
        ...
        self.tmpdir = f'{config.homedir}/tmp'
        Path(self.tmpdir).mkdir(parents=True,exist_ok=True)
        self.uuid = tools.get_md5(f"{time.time()}{role}{rate}{volume}{pitch}{voice_autorate}{len(files)}{tts_type}{langcode}")
        self.end=False
