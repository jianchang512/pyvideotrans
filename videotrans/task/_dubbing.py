...
class DubbingSrt(BaseTask):
    ...
    def __init__(self,config_params:Dict=None,obj:Dict=None):
        ...
        # 配音文件
        self.config_params['target_wav']=self.config_params['target_dir']+f'/{self.config_params["noextname"]}.{self.config_params["out_ext"]}'
        print(f"{self.config_params['target_wav']=}")
    ...
    def align(self)->None:
        ...
        try:
            ...
            if config.settings['force_edit_srt']:
                ...
                # 字幕保存到目标文件夹
                Path(self.config_params['target_sub']+"-AlignToAudio.srt").write_text(srt.strip(), encoding="utf-8", errors="ignore")
        except Exception as e:
            self.hasend = True
            tools.send_notification(str(e), f'{self.config_params["basename"]}')
            raise LogExcept(e)
