def openwin():
    ...
    def getrole():
        ...
        for it in tmp.split("\n"):
            s = it.strip().split('#')
            if len(s) != 3:
                tools.show_error("每行都必须以英文#号分割为三部分，格式为   音频名称.wav#音频文字内容#音频语言代码")
                return
            if not s[0].endswith(".wav"):
                tools.show_error("每行都必须以英文#号分割为三部分，格式为  音频名称.wav#音频文字内容#音频语言代码 ,并且第一部分为.wav结尾的音频名称")
                return
            if s[2] not in ['zh', 'ja', 'en']:
                tools.show_error("每行必须以英文#号分割为三部分，格式为 音频名称.wav#音频文字内容#音频语言代码 ,并且第三部分语言代码只能是 zh或en或ja")
                return
            role = s[0]
