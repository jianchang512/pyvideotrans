...
class TransAPI(BaseTrans):
    ...
    def _item_task(self, data: Union[List[str], str]) -> str:
        text = quote("\n".join(data))
        requrl = f"{self.api_url}target_language={self.target_language}&source_language={self.source_code if self.source_code else ''}&text={text}&secret={config.params['trans_secret']}"
