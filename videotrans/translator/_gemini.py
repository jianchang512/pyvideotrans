    ...
    def _get_content(self,data):
        ...
        try:
            ...
            if response and response.prompt_feedback.block_reason:
                raise LogExcept(self._get_error(response.prompt_feedback.block_reason, "forbid"))

            if error.find('User location is not supported') > -1 or error.find('time out') > -1:
                raise LogExcept("当前请求ip(或代理服务器)所在国家不在Gemini API允许范围")

            if response and len(response.candidates) > 0 and response.candidates[0].finish_reason not in [0, 1]:
                raise LogExcept(self._get_error(response.candidates[0].finish_reason))
