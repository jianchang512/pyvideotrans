...
`pyvideotrans` 是一款功能强大的开源视频翻译配音工具（v4.00），能够将视频自动翻译并配上目标语言的语音。其核心设计理念是模块化、多线程流水线，通过灵活的标志位组合支持多种工作模式。
...
| **⑤ 配音** | `dubbing()` | 根据目标语言字幕内容和时间戳，调用 TTS 引擎（33 种渠道）逐条生成配音音频；支持声音克隆（从原始音频截取参考片段） |
...
定义在 `videotrans/task/_base.py:24-32`，五个标志位在 `TransCreate.__post_init__()` 中根据配置自动计算：
...
v4.00 重构了任务配置为分层继承的 `@dataclass` 体系（`videotrans/task/taskcfg.py`）：
...
| 数据类 | 文件 | 用途 |
|--------|------|------|
| `InputFile` | `task/taskcfg.py` | 输入文件元信息（name, dirname, basename, ext, uuid, target_dir），支持 dict 式访问 |
| `SignMsg` | `task/taskcfg.py` | 信号消息体（type, uuid, text），在 Worker 线程与主线程间传递 |
...
所有工作线程继承自 `BaseWorker(QThread)`（`videotrans/task/job.py:32-86`）：
...
`start_thread()`（`videotrans/task/job.py:225-264`）根据 GPU 配置动态决定各 Worker 的实例数：
...
@dataclass BaseCon                    ← videotrans/configure/base.py
    ...
    │       │                          定义 9 个阶段空方法和 5 个标志位
    │       ├── @dataclass TransCreate ← videotrans/task/trans_create.py (~1200 行核心)
...
`videotrans/configure/base.py` 定义了所有类共用的核心能力：
...
`videotrans/task/_base.py:14-69` 定义了所有任务子类的阶段空方法和共享工具：
...
`videotrans/task/trans_create.py`（约 1200 行）是完整 9 阶段处理逻辑的实现类。关键内部方法：
...
软件将配置分为三个层次（`videotrans/configure/config.py`），均为 `@dataclass`：

| 配置类 | 持久化 | 用途 | 示例字段 |
|--------|--------|------|---------|
| `AppCfg` | 纯内存 | 队列、状态、线程控制、运行时上下文 | `prepare_queue`, `exit_soft`, `stoped_uuid_set`, `current_status`, `line_roles` |
...
`_set_env()` 在模块加载时自动执行（`videotrans/configure/config.py:40-67`），设置：
...
`videotrans/process/signelobj.py` 实现了一个类级别单例的 `GlobalProcessManager`：

```
GlobalProcessManager (类级别单例)
    ├── _executor_cpu: multiprocessing.Pool
    │       workers = max(min(available_ram/4GB, 8, cpu_count), 2)  ← 基于剩余内存量计算
...
不再使用固定公式，而是通过 `psutil.virtual_memory().available` 获取当前系统剩余内存，按每 4GB 一个进程计算，限制在 2~8 之间，且不超过 `os.cpu_count()`。可通过 `settings.process_max` 手动覆盖。
...
`videotrans/configure/signal_hub.py` 实现了基于 Qt 信号的单例消息传递：
...
`videotrans/__init__.py` 提供了通用的懒加载机制：
...
def get_class(channel_id, provider_type, _ID_NAME_DICT):
...
| 模块 | 渠道数 | 定义位置 |
|------|--------|---------|
| 识别 (recognition) | 22 | `videotrans/recognition/__init__.py:47-78` |
| 翻译 (translator) | 24 | `videotrans/translator/__init__.py:60-97` |
| 配音 (tts) | 33 | `videotrans/tts/__init__.py:73-116` |
...
`BaseRecogn`（`videotrans/recognition/_base.py:60-80`）在 `__post_init__` 中对中日韩等语言进行特殊处理：
- `join_word_flag`：CJK 语言（zh, ja, ko, yu, th, km, yue）字幕词间不加空格（其他语言加空格）
- `maxlen`：CJK 语言每行最大字符数为 `settings.cjk_len`（默认 20），其他语言为 `settings.other_len`（默认 46）
...
| 模式 | 条件 | 方法 | 并发数 |
|------|------|------|--------|
| 逐行翻译 | 非 AI 渠道 | `_run_text()` → `_item_task()` | `settings.trans_thread`（默认 5） |
| 全文翻译 | AI 渠道 + `aisendsrt=True` | `_run_srt()` | `settings.aitrans_thread`（默认 20） |
...
`videotrans/task/only_one.py` 中的 `Worker` 类在**单个 QThread 内串行执行**全部 9 个阶段，通过 `uito = Signal(str, SignMsg)` 与主线程通信：
...
`videotrans/task/_rate.py`（862 行）实现了 `SpeedRate` 和 `TtsSpeedRate` 两个对齐引擎：
...
```
UI 定义层         videotrans/ui/         ← PySide6 UI 布局文件（~74 个），dark/ 资源文件
    ↓
UI 逻辑层         videotrans/component/   ← 通用组件：进度条、设置表单、字幕编辑器、实时语音识别、视频裁剪、文本比对
    ↓
窗口管理层        videotrans/winform/     ← 懒加载的 ~64 个设置/功能窗口模块
    ↓
...
`videotrans/mainwin/main_win.py`（524 行）职责：
...
**WinActionBase**（`mainwin/_actions_base.py`，584 行）提供：
...
**WinAction**（`mainwin/_actions.py`，785 行）提供：
...
`videotrans/configure/excepts.py`（382 行）定义了分层异常：
...
└── videotrans/                 # 核心业务逻辑代码
    ...
    │
    ├── task/                   # 任务处理逻辑与后台线程
    │   ├── _base.py            # ★ BaseTask 基类（9 阶段空方法 + 5 标志位 + 共享工具方法）
    │   ├── taskcfg.py          # ★ TaskCfgBase/VTT/STT/TTS/STS + InputFile + SignMsg + SrtItem
    │   ├── trans_create.py     # ★ TransCreate 完整实现（~1200 行，视频翻译核心）
    ...
    │   ├── _rate.py            # SpeedRate / TtsSpeedRate 音画对齐引擎（862 行）
    ...
    │   ├── _base.py            # ★ BaseRecogn（VAD 分割、CJK 处理、字幕合并，393 行）
    │   └── _*.py               # 22 个渠道实现（_whisper, _whisperx, _whispernet, _qwenasrlocal, _qwen3asr, _funasr 等）
    │
    ├── translator/             # 字幕翻译模块（24 个渠道）
    │   ├── __init__.py         # ★ 渠道常量、_ID_NAME_DICT、LANG_CODE、run()、is_allow_translate()（859 行）
    │   ├── _base.py            # ★ BaseTrans（MD5 缓存、逐行/全文翻译调度，175 行）
    │   └── _*.py               # 24 个渠道实现（_google, _chatgpt, _deepseek, _gemini, _deepl, _baidu 等）
    │
    ├── tts/                    # 文本转语音 (TTS) 模块（33 个渠道）
    │   ├── __init__.py         # ★ 渠道常量 ID、_ID_NAME_DICT、SUPPORT_CLONE、CHANGE_BY_LANGUAGE、run()（182 行）
    │   ├── _base.py            # ★ BaseTTS（异步/多线程并发调度，293 行）
    │   └── _*.py               # 33 个渠道实现（_edgetts, _openaitts, _azuretts, _gptsovits, _cosyvoice 等）
    │
    ├── process/                # 独立子进程实现
    │   ├── __init__.py         # 子进程函数导出
    │   ├── signelobj.py        # ★ GlobalProcessManager（CPU/GPU 双进程池，114 行）
    ...
    │
    ├── mainwin/                # 主窗口界面与业务逻辑
    │   ├── main_win.py         # ★ MainWindow(QMainWindow) 初始化、信号绑定、线程启动（524 行）
    │   ├── _actions.py         # ★ WinAction 核心控制器（检查、启动、状态更新，785 行）
    │   └── _actions_base.py    # ★ WinActionBase 基类（代理、模式切换、CUDA、文件选择，584 行）
    │
    ├── component/              # UI 通用组件
