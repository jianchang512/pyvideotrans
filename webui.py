...
# 语言常量
# ---------------------------------------------------------------------------
def _webui_locale(value):
    """Normalize WebUI language aliases before the core loads its catalog."""
    return {"fr": "fr_FR", "fr-FR": "fr_FR", "zh": "zh_CN", "en": "en_US"}.get(value, value)


os.environ["PYVIDEOTRANS_LANG"] = _webui_locale(
    os.environ.get("PYVIDEOTRANS_LANG", "zh_CN")
)
...
from videotrans.configure.config import ROOT_DIR, TEMP_DIR, app_cfg, params, settings, tr as _core_tr
...
def tr(text, *args):
    """Use the core catalog, formatting source text too when a key is absent."""
    translated = _core_tr(text)
...
SUBTITLE_TYPES = {tr("不嵌入字幕"): 0, tr("嵌入硬字幕"): 1, tr("嵌入软字幕"): 2, tr("嵌入硬字幕(双语)"): 3, tr("嵌入软字幕(双语)"): 4}
...
CHANNEL_SETTINGS = {
    ...
    tr("ChatGPT 翻译"): {
        "category": tr("字幕翻译渠道"),
        "fields": [
            {"key": "chatgpt_api", "label": "API URL", "type": "text", "default": "", "placeholder": tr("留空使用官方API")},
            {"key": "chatgpt_key", "label": tr("SK 密钥"), "type": "text", "default": "", "placeholder": "API Key"},
            {"key": "chatgpt_max_token", "label": tr("最大输出 Token"), "type": "text", "default": "8192"},
            {"key": "chatgpt_model", "label": tr("模型"), "type": "text", "default": "gpt-4o-mini", "placeholder": tr("输入模型名称")},
        ],
    },
    tr("DeepSeek 翻译"): {
        "category": tr("字幕翻译渠道"),
        "fields": [
            {"key": "deepseek_key", "label": tr("SK 密钥"), "type": "text", "default": "", "placeholder": "API Key"},
            {"key": "deepseek_model", "label": tr("模型"), "type": "text", "default": "deepseek-chat", "placeholder": tr("输入模型名称")},
            {"key": "deepseek_max_token", "label": tr("最大输出 Token"), "type": "text", "default": "8192"},
        ],
    },
    tr("Gemini 翻译"): {
        "category": tr("字幕翻译渠道"),
        "fields": [
            ...
            {"key": "gemini_model", "label": tr("模型"), "type": "text", "default": "gemini-2.5-flash", "placeholder": tr("输入模型名称")},
            {"key": "gemini_maxtoken", "label": tr("最大 Token"), "type": "text", "default": "8192"},
        ],
    },
    tr("AzureGPT 翻译"): {
        "category": tr("字幕翻译渠道"),
        "fields": [
            ...
            {"key": "azure_model", "label": tr("模型"), "type": "text", "default": "gpt-4o-mini", "placeholder": tr("输入模型名称")},
        ],
    },
    tr("本地大模型 (LocalLLM)"): {
        "category": tr("字幕翻译渠道"),
        "fields": [
            {"key": "localllm_api", "label": "API URL", "type": "text", "default": "http://127.0.0.1:11434/v1", "placeholder": tr("如 http://127.0.0.1:11434/v1")},
            {"key": "localllm_key", "label": tr("SK 密钥"), "type": "text", "default": "no-key", "placeholder": tr("通常填 no-key")},
            {"key": "localllm_max_token", "label": tr("最大输出 Token"), "type": "text", "default": "8192"},
            {"key": "localllm_model", "label": tr("模型"), "type": "text", "default": "", "placeholder": tr("输入模型名称")},
        ],
    },
    tr("DeepL 翻译"): {
        "category": tr("字幕翻译渠道"),
        "fields": [
            ...
            {"key": "deepl_api", "label": tr("API URL (第三方)"), "type": "text", "default": "", "placeholder": tr("留空使用官方API")},
            {"key": "deepl_gid", "label": tr("术语表 ID"), "type": "text", "default": ""},
        ],
    },
    tr("百度翻译"): {
        "category": tr("字幕翻译渠道"),
    ...
    tr("阿里百炼 (QwenMT)"): {
        "category": tr("字幕翻译渠道"),
        "fields": [
            {"key": "qwenmt_key", "label": tr("百炼 SK"), "type": "text", "default": ""},
            {"key": "qwenmt_model", "label": tr("翻译模型"), "type": "text", "default": "qwen-mt-plus", "placeholder": tr("需以 qwen-mt 开头")},
            {"key": "qwenmt_asr_model", "label": tr("语音识别模型"), "type": "text", "default": "qwen3-asr-flash", "placeholder": tr("需以 qwen3-asr 开头")},
        ],
    },
    tr("字节火山 (VolcEngine)"): {
        "category": tr("字幕翻译渠道"),
        "fields": [
            {"key": "zijiehuoshan_key", "label": tr("SK 密钥"), "type": "text", "default": ""},
            {"key": "zijiehuoshan_model", "label": tr("推理接入点"), "type": "text", "default": "", "placeholder": tr("输入接入点名称")},
        ],
    },
    tr("MiniMax 翻译"): {
        "category": tr("字幕翻译渠道"),
        "fields": [
            {"key": "minimax_key", "label": tr("SK 密钥"), "type": "text", "default": ""},
            {"key": "minimax_api", "label": "API URL", "type": "text", "default": "api.minimax.io"},
            {"key": "minimax_model", "label": tr("模型"), "type": "text", "default": "MiniMax-M3", "placeholder": tr("输入模型名称")},
            {"key": "minimax_max_tokens", "label": tr("最大输出 Token"), "type": "text", "default": "8192"},
        ],
    },
    tr("智谱 AI 翻译"): {
        "category": tr("字幕翻译渠道"),
        "fields": [
            ...
            {"key": "zhipu_model", "label": tr("模型"), "type": "text", "default": "glm-4-flash", "placeholder": tr("输入模型名称")},
            {"key": "zhipu_max_token", "label": tr("最大输出 Token"), "type": "text", "default": "8192"},
        ],
    },
    tr("硅基流动 (SiliconFlow)"): {
        "category": tr("字幕翻译渠道"),
        "fields": [
            ...
            {"key": "guiji_model", "label": tr("模型"), "type": "text", "default": "Qwen/Qwen3-32B", "placeholder": tr("输入模型名称")},
            {"key": "guiji_max_token", "label": tr("最大输出 Token"), "type": "text", "default": "8192"},
        ],
    },
    tr("OpenRouter 翻译"): {
        "category": tr("字幕翻译渠道"),
        "fields": [
            {"key": "openrouter_key", "label": tr("SK 密钥"), "type": "text", "default": ""},
            {"key": "openrouter_model", "label": tr("模型"), "type": "text", "default": "", "placeholder": tr("输入模型名称")},
            {"key": "openrouter_max_token", "label": tr("最大输出 Token"), "type": "text", "default": "8192"},
        ],
    },
    tr("小米 AI 翻译"): {
        "category": tr("字幕翻译渠道"),
        "fields": [
            {"key": "xiaomi_key", "label": tr("小米 Key"), "type": "text", "default": ""},
            {"key": "xiaomi_model", "label": tr("模型"), "type": "text", "default": "mimo-v2.5-pro", "placeholder": tr("输入模型名称")},
            {"key": "xiaomi_maxtoken", "label": tr("最大 Token"), "type": "text", "default": "8192"},
        ],
    },
    ...
    "OpenAI ASR": {
        "category": tr("语音识别渠道"),
        "fields": [
            {"key": "openairecognapi_url", "label": "API URL", "type": "text", "default": "", "placeholder": tr("留空使用官方API")},
            {"key": "openairecognapi_key", "label": tr("SK 密钥"), "type": "text", "default": ""},
            {"key": "openairecognapi_model", "label": tr("模型"), "type": "text", "default": "whisper-1", "placeholder": tr("输入模型名称")},
        ],
    },
    "Deepgram ASR": {
        "category": tr("语音识别渠道"),
    ...
    "OpenAI TTS": {
        "category": tr("配音渠道"),
        "fields": [
            {"key": "openaitts_api", "label": "API URL", "type": "text", "default": "", "placeholder": tr("留空使用官方API")},
            {"key": "openaitts_key", "label": tr("SK 密钥"), "type": "text", "default": ""},
            {"key": "openaitts_model", "label": tr("模型"), "type": "text", "default": "tts-1", "placeholder": tr("输入模型名称")},
        ],
    },
    "Azure TTS": {
        "category": tr("配音渠道"),
        "fields": [
            ...
            {"key": "azure_speech_region", "label": "Region / URL", "type": "text", "default": "eastasia", "placeholder": tr("如 eastasia 或完整URL")},
        ],
    },
    "ElevenLabs TTS": {
        "category": tr("配音渠道"),
    ...
    "CosyVoice TTS": {
        ...
        "fields": [
            ...
            {"key": "cosyvoice_instruct_text", "label": tr("Prompt 提示词"), "type": "text", "default": ""},
        ],
    },
    ...
    tr("阿里百炼 TTS (Qwen-TTS)"): {
        "category": tr("配音渠道"),
        "fields": [
            {"key": "qwentts_key", "label": tr("百炼 SK"), "type": "text", "default": ""},
            {"key": "qwentts_model", "label": tr("模型"), "type": "text", "default": "qwen3-tts-flash", "placeholder": tr("输入模型名称")},
        ],
    },
    tr("Qwen-TTS 本地"): {
        "category": tr("配音渠道"),
        "fields": [
            {"key": "qwenttslocal_prompt", "label": tr("自定义语音提示词"), "type": "text", "default": ""},
        ],
    },
    tr("豆包语音合成 2.0"): {
        "category": tr("配音渠道"),
    ...
    "Minimaxi TTS": {
        "category": tr("配音渠道"),
        "fields": [
            {"key": "minimaxi_apikey", "label": tr("SK 密钥"), "type": "text", "default": ""},
    ...
    "X.AI TTS": {
        "category": tr("配音渠道"),
        "fields": [
            {"key": "xaitts_key", "label": tr("SK 密钥"), "type": "text", "default": ""},
        ],
    },
    tr("小米 TTS"): {
        "category": tr("配音渠道"),
...
def build_ass_editor():
    ...
    with gr.Accordion(tr("🎨 硬字幕样式编辑"), open=False):
        ...
        with gr.Tabs():
            with gr.Tab(tr("主字幕")):
                with gr.Row():
                    ass_fontname = gr.Textbox(label=tr("字体名称"), value=style.get('Fontname', 'Arial'))
                    ass_fontsize = gr.Slider(label=tr("字体大小"), minimum=1, maximum=200, value=style.get('Fontsize', 16), step=1)
                with gr.Row():
                    ass_primary_color = gr.ColorPicker(label=tr("主颜色"), value=_parse_ass_color(style.get('PrimaryColour', '&H00FFFFFF&')))
                    ass_outline_color = gr.ColorPicker(label=tr("描边颜色"), value=_parse_ass_color(style.get('OutlineColour', '&H00000000&')))
                    ass_back_color = gr.ColorPicker(label=tr("背景颜色"), value=_parse_ass_color(style.get('BackColour', '&H00000000&')))
                with gr.Row():
                    ass_bold = gr.Checkbox(label=tr("粗体"), value=bool(style.get('Bold', 0)))
                    ass_italic = gr.Checkbox(label=tr("斜体"), value=bool(style.get('Italic', 0)))
                    ass_underline = gr.Checkbox(label=tr("下划线"), value=bool(style.get('Underline', 0)))
                    ass_strikeout = gr.Checkbox(label=tr("删除线"), value=bool(style.get('StrikeOut', 0)))
            with gr.Tab(tr("底部字幕（双语时）")):
                with gr.Row():
                    ass_bottom_fontname = gr.Textbox(label=tr("字体名称"), value=style.get('Bottom_Fontname', 'Arial'))
                    ass_bottom_fontsize = gr.Slider(label=tr("字体大小"), minimum=1, maximum=200, value=style.get('Bottom_Fontsize', 16), step=1)
                with gr.Row():
                    ass_bottom_primary_color = gr.ColorPicker(label=tr("主颜色"), value=_parse_ass_color(style.get('Bottom_PrimaryColour', '&H00FFFFFF&')))
                    ass_bottom_outline_color = gr.ColorPicker(label=tr("描边颜色"), value=_parse_ass_color(style.get('Bottom_OutlineColour', '&H00000000&')))
                    ass_bottom_back_color = gr.ColorPicker(label=tr("背景颜色"), value=_parse_ass_color(style.get('Bottom_BackColour', '&H00000000&')))
                with gr.Row():
                    ass_bottom_bold = gr.Checkbox(label=tr("粗体"), value=bool(style.get('Bottom_Bold', 0)))
                    ass_bottom_italic = gr.Checkbox(label=tr("斜体"), value=bool(style.get('Bottom_Italic', 0)))
            with gr.Tab(tr("全局样式")):
                with gr.Row():
                    ass_border_style = gr.Dropdown(label=tr("边框样式"), choices=[tr("描边"), tr("不透明背景")], value=tr("描边") if style.get('BorderStyle', 1) == 1 else tr("不透明背景"))
                    ass_outline = gr.Slider(label=tr("描边粗细"), minimum=0.0, maximum=10.0, value=style.get('Outline', 0.5), step=0.1)
                    ass_shadow = gr.Slider(label=tr("阴影"), minimum=0.0, maximum=10.0, value=style.get('Shadow', 0.5), step=0.1)
                with gr.Row():
                    ass_scale_x = gr.Slider(label=tr("水平缩放 %"), minimum=1, maximum=1000, value=style.get('ScaleX', 100), step=1)
                    ass_scale_y = gr.Slider(label=tr("垂直缩放 %"), minimum=1, maximum=1000, value=style.get('ScaleY', 100), step=1)
                    ass_spacing = gr.Slider(label=tr("字间距"), minimum=-100, maximum=100, value=style.get('Spacing', 0), step=1)
                    ass_angle = gr.Slider(label=tr("旋转角度"), minimum=-360, maximum=360, value=style.get('Angle', 0), step=1)
                with gr.Row():
                    ass_margin_l = gr.Slider(label=tr("左边距"), minimum=0, maximum=1000, value=style.get('MarginL', 10), step=1)
                    ass_margin_r = gr.Slider(label=tr("右边距"), minimum=0, maximum=1000, value=style.get('MarginR', 10), step=1)
                    ass_margin_v = gr.Slider(label=tr("垂直边距"), minimum=0, maximum=1000, value=style.get('MarginV', 10), step=1)
                ass_alignment = gr.Dropdown(label=tr("对齐位置"), choices=[tr("左下"), tr("中下"), tr("右下"), tr("左中"), tr("正中"), tr("右中"), tr("左上"), tr("中上"), tr("右上")],
                    value={1: tr("左下"), 2: tr("中下"), 3: tr("右下"), 4: tr("左中"), 5: tr("正中"), 6: tr("右中"), 7: tr("左上"), 8: tr("中上"), 9: tr("右上")}.get(style.get('Alignment', 2), tr("中下")))
        with gr.Row():
        ...
        def save_ass_style(fontname, fontsize, primary_color, outline_color, back_color, bold, italic, underline, strikeout,
            ...
            am = {tr("左下"): 1, tr("中下"): 2, tr("右下"): 3, tr("左中"): 4, tr("正中"): 5, tr("右中"): 6, tr("左上"): 7, tr("中上"): 8, tr("右上"): 9}
            _save_ass_style({
                ...
                'BorderStyle': 1 if border_style == tr("描边") else 3, 'Outline': float(outline), 'Shadow': float(shadow),
        ...
        def reset_ass_style():
            ...
            return (s['Fontname'], s['Fontsize'], _parse_ass_color(s['PrimaryColour']), _parse_ass_color(s['OutlineColour']),
                    ...
                    {1: tr("左下"), 2: tr("中下"), 3: tr("右下"), 4: tr("左中"), 5: tr("正中"), 6: tr("右中"), 7: tr("左上"), 8: tr("中上"), 9: tr("右上")}.get(s['Alignment'], tr("中下")),
                    tr("✅ 已恢复默认样式"))
...
def build_channel_settings():
    ...
    gr.Markdown(tr("### 渠道设置"))
    gr.Markdown(tr("配置各渠道的 API 地址、SK 密钥等信息。**保存后与桌面版 (sp.exe) 通用**，配置文件存储在 `videotrans/params.json` 中。"))
    ...
    with gr.Tabs():
        ...
        with gr.Tab(tr("设置参考音频")):
            ...
            gr.Markdown(
                tr('配置声音克隆（clone）使用的参考音频。每行一条，格式为：`文件名.wav#音频中的说话文本`\n- 音频文件需放在 `{}/f5-tts/` 目录下\n- 文件格式必须为 wav\n- 每行用 `#` 分隔文件名和对应文本', ROOT_DIR)
            )

            ref_audio_text = gr.Textbox(
                label=tr("参考音频列表"),
                value=str(_safe_get("f5tts_role", "")),
                placeholder=tr("myaudio1.wav#你说四大皆空，却为何紧闭双眼\nmyaudio2.wav#Hello, this is a test audio"),
                lines=8,
                interactive=True,
            )
            ...
            def save_ref_audio(text):
                ...
                for i, line in enumerate(lines):
                    ...
                    parts = line.split("#")
                    if len(parts) != 2:
                        errors.append(tr('第 {} 行格式错误，需用 # 分隔文件名和文本', i + 1))
                        continue
                    ...
                    if not (f5tts_dir / filename).exists() and not (f5tts_dir / f"{filename}.wav").exists():
                        errors.append(tr('第 {} 行：文件 `{}` 在 f5-tts/ 目录下不存在', i + 1, filename))
                        continue
...
_prompt_labels = {k: tr('whisper {} 提示词', k.replace('initial_prompt_', '')) for k in _prompt_keys_list}
...
def _save_section(section_key, keys):
    ...
    import gradio as gr
    with gr.Row():
        save_btn = gr.Button(tr('💾 保存 {}', ADVANCED_SECTION_TITLES.get(section_key, section_key)), variant="primary", size="sm")
...
ADVANCED_SECTION_TITLES = {
    "common": tr("通用设置"), "video": tr("视频输出控制"), "whisper": tr("语音识别参数"),
    "trans": tr("字幕翻译调整"), "dubbing": tr("字幕配音调整"),
    "justify": tr("字幕声音画面对齐"), "prompt_init": tr("Whisper模型提示词"),
}


def build_advanced_settings():
    import gradio as gr
    gr.Markdown(tr("配置全局高级参数。**保存后与桌面版 (sp.exe) 通用**，配置文件存储在 `videotrans/cfg.json` 中。\n⚠️ 部分参数修改后需要**重启软件**才能生效。"))
    ...
    with gr.Accordion(tr("📋 字幕配音调整"), open=False):
        ...
        with gr.Row():
            _w("edgetts_max_concurrent_tasks", tr("EdgeTTS并发数"), tr("越大越快但可能限流"))
            ...
            with gr.Tab(tr("🎬 视频翻译"), id="translate"):
                ...
                with gr.Row():
                    with gr.Column(scale=3):
                        input_file = gr.Video(label=tr("选择视频文件"), interactive=True)

                        recogn_choice = gr.Dropdown(choices=RECOGN_NAMES, value=RECOGN_NAMES[int(_user_params.get('recogn_type', DEFAULT_RECOGN)) if str(_user_params.get('recogn_type', '')).isdigit() else DEFAULT_RECOGN], label=tr("识别渠道"), interactive=True)
                        model_choice = gr.Dropdown(choices=FASTER_MODEL_NAMES, value=_user_params.get('model_name', DEFAULT_MODEL), label=tr("模型"), interactive=True)

                        translate_choice = gr.Dropdown(choices=TRANSLATE_NAMES, value=TRANSLATE_NAMES[int(_user_params.get('translate_type', DEFAULT_TRANSLATE)) if str(_user_params.get('translate_type', '')).isdigit() else DEFAULT_TRANSLATE], label=tr("翻译渠道"), interactive=True)
                        source_lang = gr.Dropdown(choices=LANG_CHOICES, value=_lang_code_from_display(_user_params.get('source_language', DEFAULT_SOURCE_LANG)), label=tr("发音语言（源语言）"), interactive=True)
                        target_lang = gr.Dropdown(choices=[('-', '-')]+LANG_CHOICES, value=_lang_code_from_display(_user_params.get('target_language', DEFAULT_TARGET_LANG)), label=tr("目标语言"), interactive=True)

                        tts_choice = gr.Dropdown(choices=TTS_NAMES, value=TTS_NAMES[int(_user_params.get('tts_type', DEFAULT_TTS)) if str(_user_params.get('tts_type', '')).isdigit() else DEFAULT_TTS], label=tr("配音渠道"), interactive=True)
                        ...
                        voice_role = gr.Dropdown(choices=_init_roles, value=_init_role_val, label=tr("配音角色"), interactive=True)

                        with gr.Row():
                            voice_autorate = gr.Checkbox(label=tr("配音加速"), value=True)
                            video_autorate = gr.Checkbox(label=tr("视频慢速"), value=False)
                        with gr.Row():
                            voice_rate = gr.Slider(minimum=-50, maximum=50, value=int(str(_user_params.get("voice_rate", "0")).replace("%","")), step=1, label=tr("配音语速 (%)"))
                            volume_rate = gr.Slider(minimum=-95, maximum=100, value=int(str(_user_params.get("volume", "0")).replace("%","")), step=1, label=tr("音量调整 (%)"))
                            pitch_rate = gr.Slider(minimum=-100, maximum=100, value=int(str(_user_params.get("pitch", "0")).replace("Hz","")), step=1, label=tr("音调 (Hz)"))
                        subtitle_type = gr.Dropdown(choices=list(SUBTITLE_TYPES.keys()), value=list(SUBTITLE_TYPES.keys())[int(_user_params.get('subtitle_type', 1)) if str(_user_params.get('subtitle_type', '')).isdigit() and int(_user_params.get('subtitle_type', 1)) < len(SUBTITLE_TYPES) else 1], label=tr("字幕嵌入类型"), interactive=True)
                        build_ass_editor()

                        with gr.Accordion(tr("📋 更多设置"), open=False):
                            with gr.Row():
                                remove_noise = gr.Checkbox(label=tr("降噪"), value=False)
                                fix_punc = gr.Dropdown(choices=list(PUNC_OPTIONS.keys()), value=tr("默认标点"), label=tr("标点处理"), interactive=True)
                            with gr.Row():
                                is_separate = gr.Checkbox(label=tr("分离人声背景声"), value=False)
                                embed_bgm = gr.Checkbox(label=tr("重新嵌入背景声"), value=True)
                            with gr.Row():
                                loop_bgm = gr.Dropdown(choices=list(LOOP_BGM_OPTIONS.keys()), value=tr("背景音截断"), label=tr("背景音处理"), interactive=True)
                                backaudio_volume = gr.Slider(minimum=0.0, maximum=2.0, value=float(_user_params.get("backaudio_volume", settings.get("backaudio_volume", 0.8))), step=0.1, label=tr("背景音量"))

                        cuda_accel = gr.Checkbox(label=tr("启用 CUDA 加速"), value=False)
                        channel_warning = gr.Markdown("", visible=False)
                        
                        start_btn = gr.Button(tr("🚀 开始执行"), variant="primary", size="lg")

                    with gr.Column(scale=2):
                        log_output = gr.Textbox(label=tr("执行日志"), lines=20, interactive=False)
                        video_preview = gr.Video(label=tr("视频预览"), interactive=False)
                        result_files = gr.File(label=tr("输出文件（点击下载）"), interactive=False)
                ...
                def run_translation(file_path, recogn_display, model_name, translate_display,
                    ...
                    try:
                        ...
                        yield log(tr('源文件: {}', Path(file_path).name)), None, [], _BTN_RUNNING
                        ...
                        yield log(tr('识别: {}  翻译: {}  配音: {}', RECOGN_NAMES[recogn_idx], TRANSLATE_NAMES[translate_idx], TTS_NAMES[tts_idx])), None, [], _BTN_RUNNING
                        yield log(tr('语言: {} → {}  角色: {}', source_code, target_code, voice_role_name)), None, [], _BTN_RUNNING
                        ...
                        stages = [
                            ...
                            (tr("阶段 3/8: 说话人分离..."), "diariz", tr("说话人分离完成")),
                            (tr("阶段 4/8: 字幕翻译..."), "trans", tr("字幕翻译完成")),
                            (tr("阶段 5/8: 配音生成..."), "dubbing", tr("配音生成完成")),
                            (tr("阶段 6/8: 音画对齐..."), "align", tr("音画对齐完成")),
                            (tr("阶段 7/8: 二次识别..."), "recogn2pass", tr("二次识别完成")),
                            (tr("阶段 8/8: 最终合成..."), "assembling", tr("最终合成完成")),
                        ]
                        ...
                        yield log(tr('输出目录: {}', _target_dir)), video_preview_path, output_files, _BTN_IDLE

                    except Exception as e:
                        tb = traceback.format_exc()
                        yield log(tr('❌ 执行出错: {}\n\n{}', str(e), tb)), None, [], _BTN_IDLE
